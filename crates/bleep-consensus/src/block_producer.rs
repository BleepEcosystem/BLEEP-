...
impl BlockProducer {
    ...
    pub fn new_with_sig_availability(
        ...
        let bench = PLMutex::new(PerformanceBenchmark::new(
            NUM_SHARDS,
    ...
    async fn produce_one(&self) -> Result<Option<FinalizedBlock>, String> {
        ...
        let pending = self
            .tx_pool
            .peek_for_block(self.config.max_txs_per_block)
            .await;
        if pending.is_empty() {
            return Ok(None);
        }
